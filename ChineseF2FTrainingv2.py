...
FOLDER_NAME = 'justfont_400_bigh2'
PATH = 'E:/P2PFontsData/'+FOLDER_NAME+'/'
EPOCHS = 300
TestDataCnt = 30
...
inp, re = load(PATH+'A/1-06.jpg',PATH+'B/1-06.jpg')
...
checkpoint_dir = './'+FOLDER_NAME+'/training_checkpoints'
...
def generate_images(model, test_input, tar):
  ...
  plt.axis('off')
  ImgCnt = ImgCnt + 1
  plt.savefig(FOLDER_NAME+'/'+str(ImgCnt%TestDataCnt)+'/'+str(ImgCnt%TestDataCnt)+'-'+str(floor(ImgCnt/TestDataCnt))+'.png')
  #release memory
  fig.clf()
  plt.close()
  gc.collect()
# Test the function  
for example_input, example_target in test_dataset.take(TestDataCnt):
...
def fit(train_ds, epochs, test_ds):
  for epoch in range(epochs):
    ...
    for example_input, example_target in test_ds.take(TestDataCnt):
      generate_images(generator, example_input, example_target)
    print("Epoch: ", epoch)
